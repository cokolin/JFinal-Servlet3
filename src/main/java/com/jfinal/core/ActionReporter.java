...
public final class ActionReporter {
  ...
  static final boolean isMultipartRequest(HttpServletRequest request){
    String content_type = request.getContentType();
    return content_type != null && content_type.toLowerCase().indexOf("multipart") > -1;
  }
	...
	static final boolean reportCommonRequest(Controller controller, Action action) {
		if (isMultipartRequest(controller.getRequest())) {	// if (content_type == null || content_type.indexOf("multipart/form-data") == -1) {
	...
	private static final void doReport(Controller controller, Action action) {
		StringBuilder sb = new StringBuilder("JFinal action report --------------------------------------\n");
