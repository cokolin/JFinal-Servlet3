...
public abstract class Controller {
  private static final Logger log = LoggerFactory.getLogger(Controller.class);
	...
	/**
	 * Get upload files from multipart request.
	 * @return
	 */
	public Collection<Part> getFiles() {
		try {
	...
	/**
	 * Get upload file by parameter name.
	 * @param parameterName
	 * @return
	 */
	public Part getFile(String parameterName) {
		try {
      return request.getPart(parameterName);
    } catch (IOException e) {
      log.warn(e.getMessage(), e);
      return null;
    } catch (ServletException e) {
  ...
  public Part getFile() {
    Collection<Part> uploadFiles = getFiles();
    return uploadFiles.isEmpty() ? null : uploadFiles.iterator().next();
  }
